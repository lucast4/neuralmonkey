...
def plot_timecourse_all(Dc, PAprobs, savedir, YLIMS):
    ##### Plot
    LIST_PARAMS = []
    LIST_SAVESUFF = []
...
def plot_scalar_all(dfthis, savedir, var_score):
    from pythonlib.tools.plottools import savefig
    print("Saving figures to: ", savedir)
    
    for hue in [None, "date", "decoder_class_idx_in_syntax", "syntax_concrete_idx1", "syntax_concrete_idx1_drawn"]:
    ...
    for hue in [None, "date", "decoder_class_was_drawn", "decoder_class_idx_in_shapes_drawn", "syntax_concrete_idx1", "syntax_concrete_idx1_drawn"]:
        fig = sns.catplot(data=dfthis, x = "decoder_class_idx_in_syntax", y=var_score, 
                        col="bregion", col_wrap=6, kind="point", errorbar=("ci", 68), hue = hue)
        for ax in fig.axes.flatten():
            ax.axhline(0, color="k", alpha=0.5)
        savefig(fig, f"{savedir}/catplot-decoder_class_idx_in_syntax-hue={hue}-var_score={var_score}.pdf")
        plt.close("all")
    
    for x_var in ["syntax_role_v2", "syntax_role"]:
        order = sorted(dfthis[x_var].unique())

        fig = sns.catplot(data=dfthis, x = x_var, y=var_score, kind="bar", errorbar=("ci", 68), aspect=1,
                        col = "bregion", col_wrap=6, order=order)
        rotateLabel(fig)
        for ax in fig.axes.flatten():
            ax.axhline(0, color="k", alpha=0.5)
        savefig(fig, f"{savedir}/catplot-{x_var}-var_score={var_score}-1.pdf")

        fig = sns.catplot(data=dfthis, x = x_var, y=var_score, hue="decoder_class_idx_in_syntax", kind="point", errorbar=("ci", 68), aspect=1,
                        col = "bregion", col_wrap=6, order=order)
        rotateLabel(fig)
        for ax in fig.axes.flatten():
            ax.axhline(0, color="k", alpha=0.5)
        savefig(fig, f"{savedir}/catplot-{x_var}-var_score={var_score}-2.pdf")
        plt.close("all")
    ...
    fig, axes = plot_subplots_heatmap(dfthis, "decoder_class_idx_in_syntax", "syntax_concrete", var_score, "bregion",
...
def dfallpa_preprocess_condition(DFallpa, shape_var_suff="shape", loc_var_suff="loc"):
...
def probs_timecourse_normalize(Dc, PAprobs, norm_method, map_tc_to_syntaxconcrete=None, 
                               ...
                               define_shape_visible_using="task"):
    """
    Normalize time course of probabilties.

    
    PARAMS:
    - map_decoder_class_to_baseline_trials, dict, where trials are indices into PAprobs.Trials.
    RETURNS:
    ...
    if norm_method == "minus_base_twind":
        ...
        # Two approaches:
        # (1) for each decoder and trail, decide whether it was drawn based on taskconfig_shp
        # --- set map_decoder_class_to_baseline_trials = None
        # (2) for each decoder, input by hand the list of trials that constiotute its baseline. 
        # --- set map_decoder_class_to_baseline_trials.
        ...
        for idx_decoder in range(n_classes):
            ...
            if map_decoder_class_to_baseline_trials is None:
                ...
                if Dc.VarDecode == "seqc_0_shape":
                    ...
                    if map_tc_to_syntaxconcrete is not None:
                        # Then do a sanity check that variables match
                ...
                inds_base = map_decoder_class_to_baseline_trials[decoder_class]
        ...
        # Do both, in order.
        PAprobsNorm = probs_timecourse_normalize(Dc, PAprobs, "minus_not_visible_timecourse", map_tc_to_syntaxconcrete, twind_base, map_decoder_class_to_baseline_trials)
        PAprobsNorm = probs_timecourse_normalize(Dc, PAprobsNorm, "minus_base_twind", map_tc_to_syntaxconcrete, twind_base, map_decoder_class_to_baseline_trials)
...
# def _probs_timecourse_normalize(probs_mat_all, labels, norm_method, map_decoder_class_to_baseline_trials=None, 
#                                map_tc_to_syntaxconcrete=None, twind_base = None, times=None, var_train=None):
#     """
#     Normalize time course of probabilties.
...
#     assert probs_mat_all.shape[2] == len(labels)
#     if times is not None:
#         assert len(times) == probs_mat_all.shape[1]
...
#         # Get baseline timecourse for each decoder label
#         list_prob_vec = []
#         n_classes = probs_mat_all.shape[2]
#         # n_classes = len(PAprobs.Chans)
#         for idx_decoder in range(n_classes):
#             decoder_class = labels[idx_decoder]

#             if map_decoder_class_to_baseline_trials is not None:
#                 # Find all the trials where the decoder shape was not drawn
#                 if var_train == "seqc_0_shape":
#                     tmp = [decoder_class not in taskconfig for taskconfig in PAprobs.Xlabels["trials"]["taskconfig_shp"]]
#                     if map_tc_to_syntaxconcrete is not None:
#                         # Then do a sanity check that variables match
#                         tmp2 = [map_tc_to_syntaxconcrete[tc][idx_decoder]==0 for tc in PAprobs.Xlabels["trials"]["trialcode"]]
#                         assert tmp==tmp2, "bug, I expect this to be."
#                 elif var_train == "seqc_0_loc":
#                     tmp = [decoder_class not in taskconfig for taskconfig in PAprobs.Xlabels["trials"]["taskconfig_loc"]]
#                 else:
#                     print(var_train)
#                     assert False
...
#             # Get average timecoures for those trials
#             prob_vec = np.mean(probs_mat_all[inds_base, :, :], axis=1) # (ntimes)
#             assert prob_vec.shape == (probs_mat_all.shape[1], )
...
if __name__=="__main__":
    ...
    SAVEDIR_BASE = f"/lemur2/lucas/analyses/recordings/main/decode_moment/PIG/{animal}-{date}-ipynb_params_2-{classifier_ver}"
    which_level = "trial"
        ...
        if WHICH_PLOTS[0]=="1":
            ...
            for bregion in list_bregion:
                ...
                for var_train, var_test, labels_in_order_keep in [
                    ...
                    sdir = f"{SAVEDIR_BASE}/syntax_TI_plots/timecourse/var_train={var_train}-var_test={var_test}-combine={COMBINE_AREAS}/decoder_train/{bregion}"
                    os.makedirs(sdir, exist_ok=True)
                    dfscores_orig, Dc, PAtrain, PAtest = pipeline_train_test_scalar_score(DFallpa, bregion, var_train, event_train, twind_train, 
                                                                                    filterdict_train,
                                                        var_test, event_test, list_twind_test, filterdict_test, sdir,
                                                        include_null_data=include_null_data, decoder_method_index=None,
                                                        prune_labels_exist_in_train_and_test=False, PLOT=True,
                                                        which_level_test=which_level_test, classifier_version=classifier_ver)

                    # for VAR_SCORE in ["score", "score_norm_minmax"]:
                    for VAR_SCORE in ["score"]:
                        # TODO: doesnt work fro score_norm_minmax... need to do it within timeseries_score_wrapper
                        # dfscores = dfscores_orig.copy()
                        # dfscores["score"] = dfscores[VAR_SCORE]
                        # dfscores["score_min_base"] = dfscores[f"{VAR_SCORE}_min_base"] # score_norm_minmax_min_base

                        SAVEDIR = f"{SAVEDIR_BASE}/syntax_TI_plots/timecourse/var_train={var_train}-var_test={var_test}-combine={COMBINE_AREAS}/VAR_SCORE={VAR_SCORE}/{bregion}"
                        ...
                        PAprobsOrig, probs_mat_all, times, labels = Dc.timeseries_score_wrapper(PAtest, 
                        ...
                        for NORM_METHOD in [None, "minus_base_twind", "minus_not_visible_and_base"]:
                            ...
                            path = f"{savedir}/Dc.pkl"
                            with open(path, "wb") as f:
                                pickle.dump(Dc, f)

                            ##### RUN ANALY
                            ##### Plot
                            plot_timecourse_all(Dc, PAprobs, savedir, YLIMS=YLIMS)
            ...
            for test_dataset in ["pig_samp_post", "pig_samp_post_early", "pig_samp_post_late"]:
                for n_min_per_var in [8]:
                    ...
                    for bregion in list_bregion:
                        ...
                        dfscores_orig, Dc, PAtrain, PAtest = pipeline_train_test_scalar_score(DFallpa, bregion, var_train, event_train, twind_train, filterdict_train,
                        ...
                        for VAR_SCORE in ["score", "score_norm_minmax"]:
                            ...
                            dfscores["score"] = dfscores[VAR_SCORE]
                            dfscores["score_min_base"] = dfscores[f"{VAR_SCORE}_min_base"] # score_norm_minmax_min_base
                            SAVEDIR = f"{SAVEDIR_BASE}/syntax_TI_plots/scalar/train_data={train_dataset}-test_data={test_dataset}-combine_{COMBINE_AREAS}-include_null={include_null_data}-nmin={n_min_per_var}/VAR_SCORE={VAR_SCORE}"
                            os.makedirs(SAVEDIR, exist_ok=True)
