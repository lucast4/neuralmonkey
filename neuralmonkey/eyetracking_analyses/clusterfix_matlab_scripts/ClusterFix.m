...
MAX_N_PTS = 1e5; % @LT: n pts, no need more than this for computing n clusters (sillhoute). 
% 1e5 is empriclaly determined to take around 2 minutes for each n clust
% (sillouste)
REDUCE_DIMENSIONS = false; %@LT added, thinking this should improve things...
...
for cndlop = 1:length(eyedat)
    if size(eyedat{cndlop},2) > 500/samprate/1000
        ...
        % ==== @LT: Subsample points, as before take 1/10, unless you have
        % more data than MAX_N_PTS*10, in which case take MAX_N_PTS amount
        % of pts for subsmaple, evenly distributed over the entire
        % timepoints.
        npts = size(points,1);
        if npts<MAX_N_PTS*10
            % Identical to original code, take 1/10th of data.
            npts_take = floor(npts/10);
        else
            % Subsample to a fixed num pts.
            npts_take = MAX_N_PTS;
        end
        disp(['-- This many datapts total: ', num2str(npts)]);
        disp(['-- Downsampling points for sillhoute score, to: ', num2str(npts_take)]);
        indstake = linspace(1, npts, npts_take); % evenly spaced pts.
        indstake = unique(floor(indstake)); % make into unique integers
        points_sub = points(indstake, :);    
        
        if REDUCE_DIMENSIONS
            % Original method
            points_sub = points_sub(:, 2:4);
        end
        ...
        for numclusts = 2:5
            ...
            disp('kmeans1');
            tic
%             T = kmeans(points(1:10:end,2:4),numclusts,'replicate',5); % @Kgg - can use fewer than 1/10
            T = kmeans(points_sub,numclusts,'replicate',5); % @LT: subsampling, but capping at MAX_N_PTS
            toc
            disp('intervsintradist');
            tic
%             [silh] = InterVSIntraDist(points(1:10:end,2:4),T);
            [silh] = InterVSIntraDist(points_sub,T); % @LT: subsampling, but capping at MAX_N_PTS
            toc
            sil(numclusts) = mean(silh);
        end
        disp('**Sillhoute results:');
        disp(sil);
        
        sil(sil > 0.9*max(sil)) = 1;
        numclusters = find(sil == max(sil));
        disp('**Using this many clusters:');
        disp(numclusters(end));
        ...
        for ii = 1:size(fixationtimes,2);
            ...
            sil = zeros(1,5);
%             disp('Local reclustering: sillhoute');
%             tic
            for numclusts = 1:5
...
%             disp('Local reclustering: kmeans on all data');
%             tic
