function datstruct_this = datstruct_merge_inner(datstruct, new_label_final)
...
%% Values that should concatenate
times_all =[];
waveforms_all =[];
amps_wf_all = [];
...
%% values diff, leave empty (or store array of all).
list_clust = [];
list_index = [];
for i=1:length(datstruct)
...
%         datstruct(1).GOOD;
datstruct_this.snr_aligned = nan;
datstruct_this.snr_not_aligned = nan;
datstruct_this.label_final = new_label_final;
datstruct_this.label_final_int = nan;
